...
function storeBirthDate(){
    const date = new Date(inputBirthYear.value,inputBirthMonth.value-1,inputBirthDay.value);
...
function isDatasOkay(){
    ...
    if(isSomeDataEmpty() || isDateInvalid() || isWeightInvalid() || isHeightInvalid()){
...
function isSomeDataEmpty(){
    if(inputName.value == "" || inputBirthDay.value == "" || inputBirthMonth.value == ""
    || inputBirthYear.value == "" || inputWeight.value == "" || inputHeight.value == ""
    || optionGender.value == ""){
...
function isDateInvalid(){
    if(isDayInvalid() || isMonthInvalid() || isYearInvalid()) {
        messageError.innerHTML += "[error] Date invalid, check day, month, and year before to send!";
        return true;
    }
    else return false;
}
...
function isDayInvalid(){
    ...
    else if((birthDay == 31) && (birthMonth !== 1 || birthMonth !== 3 || birthMonth !== 5 || birthMonth !== 7 || birthMonth !== 8 || birthMonth !== 10 || birthMonth !== 12)){
            return true;
        }
    else if((birthDay == 30 && birthMonth == 2)||(birthDay == 29 && birthMonth == 2 && birthYear%4 !== 0)){
            return true;
        }
        else{
            return false;
...
function isYearInvalid(){
    ...
    if((birthYear < 0 || birthYear > 2022 || isNaN(birthYear)) || (birthYear == 2022 && birthMonth > 10) || (birthDate > todayDate)) return true;
